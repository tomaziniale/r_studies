...
<li> A poll conducted by WIN-Gallup International surveyed 51,927 people from 57 countries. x</li>
</ol>
...
count(filter(.data = us12, response=="atheist")) / count(us12)
```
...
<li> The margin of error for the estimate of the proportion of atheists in the US in 2012 is 0.0135. x</li>
</ol>
...
inference(y = response, data = aus12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
mol12 <- filter(atheism, nationality=="Moldova")
inference(y = response, data = mol12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
...
<li> The most conservative estimate when calculating a confidence interval occurs when $p$ is set to 1. x</li>
</ol>
...
spain <- atheism %>% filter(nationality=="Spain" & (year==2005 | year==2012))
inference(y = response, data = spain, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist", x = as.factor(year))
# ME 2005
1.96 * (sqrt((0.1003*(1-0.1003))/1146))
# ME 2012
1.96 * (sqrt((0.09*(1-0.09))/1145))
# with 1% os ME at the p-hat, there's no convincing evidence of such change.
```
...
usa <- atheism %>% filter(nationality=="United States" & (year==2005 | year==2012))
inference(y = response, data = usa, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist", x = as.factor(year))
# ME 2005
1.96 * (sqrt((0.01*(1-0.01))/1002))
# ME 2012
1.96 * (sqrt((0.0499*(1-0.0499))/1002))
# Considering the ME for each year, the data provides convincing evidence of of such change.
```
