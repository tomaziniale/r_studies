...
<li> Both distributions are extremely right skewed. x</li>
<li> The IQRs of the distributions are roughly equal. </li>
</ol>
```{r habit-weight-box}
# type your code for the Question 3 here, and Knit
boxplot(weight~habit, data=nc, main="baby weight and mothers habit", ylab="Weight", ylim=c(0,11), las=1)
nc %>% filter(habit=="nonsmoker") %>% ggplot(aes(x=weight)) + geom_histogram(binwidth = 1)
nc %>% filter(habit=="smoker") %>% ggplot(aes(x=weight)) + geom_histogram(binwidth = 1)
```
...
p=0.0199, less than 5% then we reject the null hypothesis in order to the alternative.
</div>
...
<li> We are 95% confident that babies born to nonsmoker mothers are on average 0.05 to 0.58 pounds heavier at birth than babies born to smoker mothers. x</li>
</ol>
```{r habit-weight-ci}
...
inference(y = gained, x = habit, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")
inference(y = gained, x = habit, data = nc, statistic = "mean", type = "ci", method = "theoretical", conf_level = 0.99)
```
