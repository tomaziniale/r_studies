...
<li> 50\% of houses in Ames are smaller than 1,499.69 square feet. x </li>
...
```{r sampling-dist}
# type your code for the Exercise here, and Run Document
# There are 15,000 elements each one based on a sample of 50 of original data.
# The distribution is unimodal, nearly normal with the center in 1499.471
sample_means50 %>% summarise(
  x_bar = mean(x_bar),
  med = median(x_bar),
  iqr = IQR(x_bar),
  min = min(x_bar),
  max = max(x_bar),
  q25 = quantile(x_bar, 0.25),
  q50 = quantile(x_bar, 0.5)
)
```
...
<ol>
<li> Each element represents a mean square footage from a simple random sample of 10 houses. x </li>
...
<ol>
<li> The variability of the sampling distribution with the smaller sample size (`sample_means50`) is smaller than the variability of the sampling distribution with the larger sample size (`sample_means150`). x</li>
...
rep50 <- ames %>%
    ...
    summarise(x_bar = mean(area))
rep150 <- ames %>%
    rep_sample_n(size = 150, reps = 500, replace = TRUE) %>%
    summarise(x_bar = mean(area))
mean(rep50$x_bar)
mean(rep150$x_bar)
ggplot(rep50, aes(x=x_bar)) + geom_histogram(binwidth = 25)
ggplot(rep150, aes(x=x_bar)) + geom_histogram(binwidth = 25)
```
